...
export function loadConfig(env = process.env) {
  ...
  return Object.freeze({
    ...
    yandexWeatherApiKey: String(env.YANDEX_WEATHER_API_KEY || '').trim(),
    yandexWeatherBaseUrl: required('YANDEX_WEATHER_BASE_URL', env.YANDEX_WEATHER_BASE_URL || (testMode ? 'https://api.weather.yandex.ru' : '')),
    metNoWeatherBaseUrl: required('MET_NO_WEATHER_BASE_URL', env.MET_NO_WEATHER_BASE_URL || (testMode ? 'https://api.met.no' : '')),
    weatherProviderOrder: Object.freeze(String(env.WEATHER_PROVIDER_ORDER || (testMode ? 'yandex,met-no,open-meteo' : '')).split(',').map((value) => value.trim()).filter(Boolean)),
