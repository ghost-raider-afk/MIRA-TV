...
export function loadConfig(env = process.env) {
  ...
  return Object.freeze({
    appName: required('APP_NAME', env.APP_NAME), appVersion: required('MIRA_TV_VERSION', env.MIRA_TV_VERSION),
    nodeEnv: required('NODE_ENV', env.NODE_ENV), host: required('HOST', env.HOST), port: integer('PORT', env.PORT, { minimum: 1, maximum: 65535 }),
    domain: required('MIRA_TV_DOMAIN', env.MIRA_TV_DOMAIN), bootstrapAdmin: bootstrapAdministrator(env, passwordMinLength),
    sessionSecret: generatedValue('SESSION_SECRET', env.SESSION_SECRET, 32), sessionTtlHours: integer('SESSION_TTL_HOURS', env.SESSION_TTL_HOURS, { minimum: 1, maximum: 168 }), secureCookies: boolean('SECURE_COOKIES', env.SECURE_COOKIES),
    ...
    sceneVideoEnabled: boolean('SCENE_VIDEO_ENABLED', env.SCENE_VIDEO_ENABLED || 'false'),
    sceneVideoFps: integer('SCENE_VIDEO_FPS', env.SCENE_VIDEO_FPS || '25', { minimum: 10, maximum: 30 }),
    sceneVideoDurationSeconds: integer('SCENE_VIDEO_DURATION_SECONDS', env.SCENE_VIDEO_DURATION_SECONDS || '12', { minimum: 4, maximum: 30 }),
    sceneVideoChromiumPath: String(env.SCENE_VIDEO_CHROMIUM_PATH || '/usr/bin/chromium'),
    ...
    metNoWeatherBaseUrl: required('MET_NO_WEATHER_BASE_URL', env.MET_NO_WEATHER_BASE_URL || (testMode ? 'https://api.met.no' : '')),
    weatherProviderOrder: Object.freeze(String(env.WEATHER_PROVIDER_ORDER || (testMode ? 'met-no,open-meteo' : '')).split(',').map((value) => value.trim()).filter(Boolean)),
    ...
    weatherGeocodingBaseUrl: required('WEATHER_GEOCODING_BASE_URL', env.WEATHER_GEOCODING_BASE_URL || (testMode ? 'https://geocoding-api.open-meteo.com' : '')),
    weatherCacheSeconds: integer('WEATHER_CACHE_SECONDS', env.WEATHER_CACHE_SECONDS || (testMode ? '600' : undefined), { minimum: 60, maximum: 7200 }),
    weatherFetchTimeoutMs: integer('WEATHER_FETCH_TIMEOUT_MS', env.WEATHER_FETCH_TIMEOUT_MS || (testMode ? '5000' : undefined), { minimum: 1000, maximum: 30000 }),
    weatherProviderCooldownSeconds: integer('WEATHER_PROVIDER_COOLDOWN_SECONDS', env.WEATHER_PROVIDER_COOLDOWN_SECONDS || (testMode ? '300' : undefined), { minimum: 30, maximum: 86400 }),
    weatherCollectorIntervalSeconds: integer('WEATHER_COLLECTOR_INTERVAL_SECONDS', env.WEATHER_COLLECTOR_INTERVAL_SECONDS || (testMode ? '60' : undefined), { minimum: 30, maximum: 3600 }),
    ...
    jsonBodyMaxBytes: integer('JSON_BODY_MAX_BYTES', env.JSON_BODY_MAX_BYTES, { minimum: 1024, maximum: 10485760 }), menuDraftMaxBytes: integer('MENU_DRAFT_MAX_BYTES', env.MENU_DRAFT_MAX_BYTES, { minimum: 1024, maximum: 10485760 }), screenSourceMaxBytes: integer('SCREEN_SOURCE_MAX_BYTES', env.SCREEN_SOURCE_MAX_BYTES, { minimum: 1024, maximum: 52428800 }),
    dashboardRefreshMinSeconds, dashboardRefreshMaxSeconds,
    screenMaxWidth: integer('SCREEN_MAX_WIDTH', env.SCREEN_MAX_WIDTH, { minimum: 320, maximum: 7680 }), screenMaxHeight: integer('SCREEN_MAX_HEIGHT', env.SCREEN_MAX_HEIGHT, { minimum: 240, maximum: 4320 }), imageMaxPixels: integer('IMAGE_MAX_PIXELS', env.IMAGE_MAX_PIXELS, { minimum: 262144, maximum: 100000000 }),
    siteAssetsRoot: required('SITE_ASSETS_ROOT', env.SITE_ASSETS_ROOT), siteLogoMaxBytes: integer('SITE_LOGO_MAX_BYTES', env.SITE_LOGO_MAX_BYTES, { minimum: 1024, maximum: 10485760 }), siteFaviconMaxBytes: integer('SITE_FAVICON_MAX_BYTES', env.SITE_FAVICON_MAX_BYTES, { minimum: 1024, maximum: 5242880 }), screenBackgroundMaxBytes: integer('SCREEN_BACKGROUND_MAX_BYTES', env.SCREEN_BACKGROUND_MAX_BYTES, { minimum: 1024, maximum: 52428800 }), sceneAssetMaxBytes: integer('SCENE_ASSET_MAX_BYTES', env.SCENE_ASSET_MAX_BYTES, { minimum: 1048576, maximum: 1073741824 }), healthReadinessCacheMs: integer('HEALTH_READINESS_CACHE_MS', env.HEALTH_READINESS_CACHE_MS, { minimum: 0, maximum: 60000 }),
    db: Object.freeze({ host: required('POSTGRES_HOST', env.POSTGRES_HOST), port: integer('POSTGRES_PORT', env.POSTGRES_PORT, { minimum: 1, maximum: 65535 }), database: required('POSTGRES_DB', env.POSTGRES_DB), user: required('POSTGRES_USER', env.POSTGRES_USER), password: generatedValue('POSTGRES_PASSWORD', env.POSTGRES_PASSWORD, 16), poolMax: integer('POSTGRES_POOL_MAX', env.POSTGRES_POOL_MAX, { minimum: 1, maximum: 100 }), connectionTimeoutMs: integer('POSTGRES_CONNECTION_TIMEOUT_MS', env.POSTGRES_CONNECTION_TIMEOUT_MS, { minimum: 100, maximum: 120000 }), idleTimeoutMs: integer('POSTGRES_IDLE_TIMEOUT_MS', env.POSTGRES_IDLE_TIMEOUT_MS, { minimum: 1000, maximum: 3600000 }) }),
