...
const PREVIOUS_KNOWN_GOOD_KEY = 'previous-known-good';
...
export async function loadPreviousKnownGood() {
  return stateValue(PREVIOUS_KNOWN_GOOD_KEY);
}
...
export async function saveLastKnownGood(record) {
  ...
  const currentRecord = await requestResult(state.get(LAST_KNOWN_GOOD_KEY));
  const current = currentRecord?.value || null;
  if (current && String(current.revision || '') !== String(record?.revision || '')) {
    state.put({ key:PREVIOUS_KNOWN_GOOD_KEY, value:current });
  }
  state.put({ key:LAST_KNOWN_GOOD_KEY, value:record });
  await transactionDone(tx);
}
...
export async function clearLastKnownGood() {
  ...
  const tx = db.transaction(STATE_STORE, 'readwrite');
  const state = tx.objectStore(STATE_STORE);
  state.delete(LAST_KNOWN_GOOD_KEY);
  state.delete(PREVIOUS_KNOWN_GOOD_KEY);
  await transactionDone(tx);
}
