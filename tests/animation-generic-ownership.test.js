...
test('animation storage owns only motion profile and scene playlist', async()=>{
 const [contract,repository]=await Promise.all([read('src/contracts/animation.js'),read('src/db/settings.js')]);
 ...
 for(const name of ['entity_json','announcement_json','brand_json','environment_json','updateAnimationEntity']) assert.doesNotMatch(repository,new RegExp(name));
