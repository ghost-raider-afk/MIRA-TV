...
test.describe('mobile application shell', () => {
  ...
  test('uses bottom navigation and an explicit section sheet', async ({ page }) => {
    ...
    await expect(rail.getByLabel('Плейлист')).toHaveCount(0);
    ...
    await rail.getByLabel('Мониторы').click();
    await expect(page).toHaveURL(/\/screens$/);
    const monitorContextTrigger = page.locator('[data-mobile-context-trigger]');
    await monitorContextTrigger.click();
    await expect(page.locator('.ui-context')).not.toHaveClass(/is-collapsed/);
