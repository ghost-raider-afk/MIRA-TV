...
test('Player exposes baked video only for the current source render revision', () => {
  const record = {
  ...
  assert.match(bakedSceneRuntimeToken(record, 13), /^fallback:/);
});

test('Render Agent architecture never adds server-side Chromium or scene encoding', async () => {
  const [server, packageService, uploadRoutes, dockerfile, docs] = await Promise.all([
    read('src/server.js'),
