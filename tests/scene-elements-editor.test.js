...
test('dedicated Scene editor is the only admin owner of generic scene elements', async () => {
  ...
  assert.match(css, /grid-template-columns:164px minmax\(0,1fr\) 276px/);
  assert.match(css, /grid-template-columns:150px minmax\(0,1fr\) 264px/);
  assert.match(elements, /onWeatherLocationSearch/);
  assert.match(scenePage, /\/api\/weather\/locations\?q=/);
  assert.match(scenePage, /refreshGeometry/);
