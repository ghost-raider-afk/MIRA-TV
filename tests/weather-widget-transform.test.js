...
test('Player shell changes rotate only the offline shell cache and preserve downloaded media data', async () => {
  ...
  assert.match(worker, /const SHELL_CACHE = 'mira-tv-player-shell-v27'/);
  assert.match(worker, /const DATA_CACHE = 'mira-tv-player-data-v18'/);
  assert.match(worker, /const RETIRED_SHELL_CACHE = 'mira-tv-player-shell-v25'/);
  assert.match(worker, /const LEGACY_SHELL_CACHE = 'mira-tv-player-shell-v26'/);
